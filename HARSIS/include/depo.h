...
template <typename Anahtar, typename Deger> class Depo {
public:
  ...
  const std::map<Anahtar, Deger> &tumunu_al() const { return m_veriler; }

  // Kayıt sayısını döner
  std::size_t boyut() const { return m_veriler.size(); }

  // Depoyu temizler
  void temizle() { m_veriler.clear(); }
  ...
  std::vector<Deger> filtrele(std::function<bool(const Deger &)> kosul) const {
    ...
    for (const auto &[anahtar, deger] : m_veriler) {
      if (kosul(deger)) {
